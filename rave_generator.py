...
class TrigramMarkovChain(object):
    ...
    def sentence(self, startword_idx=None):
        if not self.generate_ready: self.generate_prepare()
            
        if startword_idx is None:
            startword_idx = peek_random(self.start_cumsum, self.start_keys)
        yield (lambda word: word[0].upper() + word[1:])\
              (self.idx2word[startword_idx])
...
def trigram_split(filename):
    ...
    with codecs.open(filename, encoding='utf-8', mode='r') as fi:
        for line in fi:
            processed_line = reduce(lambda x, f: f(x), preprocess_conveyor, line)
...
def dump_model(filename, model):
    ...
    if not model.generate_ready: model.generate_prepare()    
...
def generate_text(file, model, words_to_generate=10000):
    import random
    import codecs
    ...
    while word_count < words_to_generate:
        ...
        paragrarh = u' '.join((sentence() for x in xrange(paragrarh_len)))
